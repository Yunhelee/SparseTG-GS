...
pretrained_generator_path = './models/netG_epoch_ImageNet_Inception_100.pth'
...
def main():
    ...
    for idx, (images, labels) in enumerate(test_loader):
        ...
        if pre.cpu().data.float() != image_names.cpu().data.float():
            ...
            continue
        length += 1
        print('The true label : ', image_names.item())
        ...
        # M2
        norm_0, norm_1, norm_2, norm_inf = Statistics(real_A, adv)
        g = cal_group(real_A, adv, segments.cpu())
        total_norm_0_m2 += norm_0
        total_norm_1_m2 += norm_1
        total_norm_2_m2 += norm_2
        total_norm_inf_m2 += norm_inf
        total_norm_g_m2 += g
        adv_noise = real_A - adv
        print('L0-2: ', torch.norm(adv_noise, 0).item())
        print('L1-2: ', torch.norm(adv_noise, 1).item())
        print('L2-2: ', torch.norm(adv_noise, 2).item())
        print('Linf-2: ', torch.norm(adv_noise, p=float('inf')).item())
        print('g-2: ', g)

        # M3
        adv_m3 = adv.clone()
        advimportance = pretrained_G(adv_m3)
        adv_m3 = reduce2(real_A, adv_m3, netT, target, adv_mask, advimportance, mask)
        logits = netT(adv_m3.to(device))
        ...
        adv_noise = real_A - adv_m3
        print('L0-3: ', torch.norm(adv_noise, 0).item())
        print('L1-3: ', torch.norm(adv_noise, 1).item())
        print('L2-3: ', torch.norm(adv_noise, 2).item())
        print('Linf-3: ', torch.norm(adv_noise, p=float('inf')).item())
        print('g-3: ', g)

        norm_0, norm_1, norm_2, norm_inf = Statistics(real_A, adv_m3)
        g = cal_group(real_A, adv_m3, segments.cpu())
        total_norm_0_m3 += norm_0
        total_norm_1_m3 += norm_1
        total_norm_2_m3 += norm_2
        total_norm_inf_m3 += norm_inf
        total_norm_g_m3 += g
        ...
        while const <= 10 and iter_num <= 20:
            ...
            adv, adv_mask, mask, boolean = increase(real_A, adv, segments, netT, target, mask, boost, reduce_prop,
                                                    adv_mask=adv_mask,
                                                    fine_tuning=True)
        ...
        norm_0, norm_1, norm_2, norm_inf = Statistics(real_A, adv)
        g = cal_group(real_A, adv, segments.cpu())
        total_norm_0_m4 += norm_0
        total_norm_1_m4 += norm_1
        total_norm_2_m4 += norm_2
        total_norm_inf_m4 += norm_inf
        total_norm_g_m4 += g

        adv_noise = real_A - adv
        print('L0-4: ', torch.norm(adv_noise, 0).item())
        print('L1-4: ', torch.norm(adv_noise, 1).item())
        print('L2-4: ', torch.norm(adv_noise, 2).item())
        print('Linf-4: ', torch.norm(adv_noise, p=float('inf')).item())
        print('g-4: ', g)

        print('The final norm 0 is : ', torch.norm(real_A - adv, 0).item())
        advimportance = pretrained_G(adv)
        adv = reduce2(real_A, adv, netT, target, adv_mask, advimportance, mask)
        ...
        norm_0, norm_1, norm_2, norm_inf = Statistics(real_A, adv)
        g = cal_group(real_A, adv, segments.cpu())
        total_norm_0_m5 += norm_0
        total_norm_1_m5 += norm_1
        total_norm_2_m5 += norm_2
        total_norm_inf_m5 += norm_inf
        total_norm_g_m5 += g
        ...
        print('L0-5: ', torch.norm(adv_noise, 0).item())
        print('L1-5: ', torch.norm(adv_noise, 1).item())
        print('L2-5: ', torch.norm(adv_noise, 2).item())
        print('Linf-5: ', torch.norm(adv_noise, p=float('inf')).item())
        print('g-5: ', g)
    ...
    print(length)
    mean_norm0_m2, mean_norm1_m2, mean_norm2_m2, mean_norminf_m2, mean_normg_m2 = average(total_norm_0_m2,
                                                                                          total_norm_1_m2,
                                                                                          total_norm_2_m2,
                                                                                          total_norm_inf_m2,
                                                                                          total_norm_g_m2, length)
    mean_norm0_m3, mean_norm1_m3, mean_norm2_m3, mean_norminf_m3, mean_normg_m3 = average(total_norm_0_m3,
                                                                                          total_norm_1_m3,
                                                                                          total_norm_2_m3,
                                                                                          total_norm_inf_m3,
                                                                                          total_norm_g_m3, length)
    mean_norm0_m4, mean_norm1_m4, mean_norm2_m4, mean_norminf_m4, mean_normg_m4 = average(total_norm_0_m4,
                                                                                          total_norm_1_m4,
                                                                                          total_norm_2_m4,
                                                                                          total_norm_inf_m4,
                                                                                          total_norm_g_m4, length)
    mean_norm0_m5, mean_norm1_m5, mean_norm2_m5, mean_norminf_m5, mean_normg_m5 = average(total_norm_0_m5,
                                                                                          total_norm_1_m5,
                                                                                          total_norm_2_m5,
                                                                                          total_norm_inf_m5,
                                                                                          total_norm_g_m5, length)

    filename = 'result_comp.txt'
    with open(filename, 'a') as file_object:
        file_object.write('-----------消 融 实 验-----------')
        file_object.write('\n The m2 inf norm are : ' + str(mean_norminf_m2) + '\n')
        file_object.write('The m2 2 norm are : ' + str(mean_norm2_m2) + '\n')
        file_object.write('The m2 1 norm are : ' + str(mean_norm1_m2) + '\n')
        file_object.write('The m2 0 norm are : ' + str(mean_norm0_m2) + '\n')
        file_object.write('The g nums are : ' + str(mean_normg_m2) + '\n')
        file_object.write('\n The m3 inf norm are : ' + str(mean_norminf_m3) + '\n')
        file_object.write('The m3 2 norm are : ' + str(mean_norm2_m3) + '\n')
        file_object.write('The m3 1 norm are : ' + str(mean_norm1_m3) + '\n')
        file_object.write('The m3 0 norm are : ' + str(mean_norm0_m3) + '\n')
        file_object.write('The g nums are : ' + str(mean_normg_m3) + '\n')
        file_object.write('\n The m4 inf norm are : ' + str(mean_norminf_m4) + '\n')
        file_object.write('The m4 2 norm are : ' + str(mean_norm2_m4) + '\n')
        file_object.write('The m4 1 norm are : ' + str(mean_norm1_m4) + '\n')
        file_object.write('The m4 0 norm are : ' + str(mean_norm0_m4) + '\n')
        file_object.write('The g nums are : ' + str(mean_normg_m4) + '\n')
        file_object.write('\n The m5 inf norm are : ' + str(mean_norminf_m5) + '\n')
        file_object.write('The m5 2 norm are : ' + str(mean_norm2_m5) + '\n')
        file_object.write('The m5 1 norm are : ' + str(mean_norm1_m5) + '\n')
        file_object.write('The m5 0 norm are : ' + str(mean_norm0_m5) + '\n')
        file_object.write('The g nums are : ' + str(mean_normg_m5) + '\n')
...
def increase(real_A, adv, segments, netT, target, mask, boost, reduce_prop=0.1, adv_mask=None, fine_tuning=False):
    Iter = 100000
    temp_eps = eps / 2
    max_iter = torch.max(segments)
    ...
    if fine_tuning:
        mask, adv_mask = perturbation_importance(real_A, adv, netT, target, adv_mask, mask, reduce_prop, segments.cpu())
        adv_noise = real_A - adv
    ...
    for iters in range(Iter):  # iter=10000
        ...
        if target.cpu().data.float() != pre.cpu().data.float():
            break
        Loss = loss_adv(logist_B, target, -kappa, False, False) / real_A.size(0)
        netT.zero_grad()
        ...
        with torch.no_grad():
            ...
            for i in range(0, max_iter * 3 - iters):
                ...
                grad_order[i] = torch.mean(temp_grad).item()
            ...
            Loss = loss_adv(logist_B, target, -100, True, False)
...
def reduce(real_A, adv, netT, target, adv_mask, advimportance, mask, group=False):
    ...
    if modi_num > 0:
        if group:
            ...
            interval_num = max(modi_num * 0.1, 100)
        reduce_idx = 0
        reduce_count = 0
        count = 0
        order = []
...
def reduce2(real_A, adv, netT, target, adv_mask, advimportance, mask, group=False):
    adv_noise = real_A - adv
    advimportance = advimportance * mask
    ...
    if modi_num > 0:
        ...
        while reduce_idx < modi_num and reduce_count < 3000:
            ...
            abs_noise = torch.abs(adv_noise).view(1, -1)
            abs_importance = torch.abs(advimportance).view(1, -1)
            reduce_mask = abs_noise != 0
            abs_importance[abs_importance == 0] = 3.

            reduce_num = torch.sum(reduce_mask).data.clone().item()
            if reduce_num == 1:
                break

            noise_show, noise_sort_idx = torch.sort(abs_importance)
            ...
            with torch.no_grad():
                ...
                ex_real_A = real_A
                ex_temp_A = ex_real_A - ex_normalized_grad
                ex_temp_A = clip(ex_temp_A, ex_real_A, eps)
                ex_adv = torch.clamp(ex_temp_A, 0, 1)
                ...
                if comp != 1:  # exists at least one adversarial sample
                    logits = netT(ex_temp_A)
                    pre = torch.argmax(logits, 1)
                    if pre != target:
                        adv = ex_temp_A
                        found = True
...
def perturbation_importance(real_A, adv, netT, target, adv_mask, mask, reduce_prop, segments=None):
    advimportance = pretrained_G(adv)
    ...
    decrease_num = int(importance_mean.size()[0] * reduce_prop)
    order = []
...
def Statistics(real_A, adv):
    adv_noise = real_A - adv
    norm_0 = torch.norm(adv_noise, p=0)
    norm_1 = torch.norm(adv_noise, p=1)
    norm_2 = torch.norm(adv_noise, p=2)
    norm_inf = torch.norm(adv_noise, p=float('inf'))
    return norm_0, norm_1, norm_2, norm_inf


def average(total_norm_0, total_norm_1, total_norm_2, total_norm_inf, total_norm_g, length):
    mean_norm_0 = total_norm_0 / length
    mean_norm_1 = total_norm_1 / length
    mean_norm_2 = total_norm_2 / length
    mean_norm_inf = total_norm_inf / length
    mean_norm_g = total_norm_g / length
    return mean_norm_0, mean_norm_1, mean_norm_2, mean_norm_inf, mean_norm_g
