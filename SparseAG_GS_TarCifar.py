...
random.seed(0)
L = random.sample(range(0, 10000), 1000)
...
def main():
    ...
    mmean_reduce = float(total_reduce) / success
    ...
    mean_running_time = float(total_time) / success
    ...
    with open(filename, 'a') as file_object:
        file_object.write('------------Target Attack CIFAR VGG-16-----------')
...
def increase(real_A, adv, segments, netT, target, mask, boost, reduce_prop=0.1, adv_mask=None, fine_tuning=False):
    ...
    for iters in range(Iter):  # iter=10000
        ...
        with torch.no_grad():
            ...
            if len(grad_order) > selnum:
                temp_mask = grad_order[0:selnum]
            else:
                temp_mask = grad_order[0:]
            selnum = len(temp_mask)
...
def reduce(real_A, adv, netT, target, adv_mask, advimportance, mask, group=False):
    if group:
        ...
        advimportance_group = advimportance * adv_mask
        importance_dict = dict()
        importance_mean = advimportance_group.mean(dim=1).mean(dim=1).mean(dim=1)
    ...
    if modi_num > 0:
        ...
        while reduce_idx < modi_num and reduce_count < 3000:
            ...
            with torch.no_grad():
                ...
                if comp != 0:  # exists at least one adversarial sample
