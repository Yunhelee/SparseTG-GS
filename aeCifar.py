...
pretrained_generator_path = './models/netG_epoch_100(cifarnet).pth'
...
def main():
    ...
    for idx, (images, labels) in enumerate(test_loader):
        ...
        adv, adv_mask, mask = increase(real_A, adv, segments, netT, target, mask, boost, adv_mask=adv_mask)
        ...
        print('L0: ', torch.norm(adv_noise, 0).item())
        print('L2: ', torch.norm(adv_noise, 2).item())
        ...
        reduce_prop = 0.1
        max_prop = 0.2
        while const <= 10 and iter_num <= 35:
            print('The current number is : ', iter_num)
            former = cal_group(real_A, adv, segments.cpu())

            advimportance = pretrained_G(adv)

            # temp_advimportance = torch.nn.functional.sigmoid(10 * activation['decoder'])
            # adv_noise = real_A - adv
            # temp = real_A - adv_noise * temp_advimportance
            # logits = model(temp.to(device))
            ...
            adv, adv_mask, mask = increase(real_A, adv, segments, netT, target, mask, boost, reduce_prop,
                                           adv_mask=adv_mask,
                                           fine_tuning=True)
        ...
        logits = model(adv.to(device))
        pred1 = torch.argmax(logits, dim=1)
        print(torch.norm(real_A - adv, 2).item())

        alpha = 1
        a = [alpha + i * 1 / 10 for i in range(0, 90)]
        for item in a:
            advimportance = torch.nn.functional.sigmoid(item * activation['decoder'])
            ...
            adv_noise = adv_noise * advimportance
            temp_adv = real_A - adv_noise
            logits = model(temp_adv.to(device))
            pred2 = torch.argmax(logits, dim=1)
            if pred1 == pred2:
                adv = temp_adv
                break
    ...
    with open(filename, 'a') as file_object:
        file_object.write('-----------消 融 实 验 CIFAR-10-----------')
...
def increase(real_A, adv, segments, netT, target, mask, boost, reduce_prop=0.1, adv_mask=None, fine_tuning=False):
    ...
    if fine_tuning:
        return adv, adv_mask, mask
    else:
        return adv, adv_mask[1:], mask
...
def CWLoss(logits, target, kappa=0, sept=False, tar=True):
    ...
    target_one_hot = Variable(torch.eye(10)[target.long()].to(device))
